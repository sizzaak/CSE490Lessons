...
void loop() {
  ...
  if(Serial.available() > 0){
    ...
    // Convert to c string and use sscanf to parse values in format i1,i2
    // then find the sum of i1 and i2
    char rcvdCstring[rcvdSerialData.length() + 1];
    strcpy(rcvdCstring, rcvdSerialData.c_str());
    int i1, i2;
    sscanf(rcvdCstring, "%d,%d", &i1,&i2);
    int sum = i1 + i2;
    String plus = "+";
    String equals = "=";
    String equationResult = i1 + plus + i2 + equals + sum;
    ...
    _display.getTextBounds(equationResult, 0, 0, &x1, &y1, &textWidth, &textHeight);
